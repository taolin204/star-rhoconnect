...
class Application < Rhoconnect::Base
  class << self
    def authenticate(username,password,session)
      ...
      # puts "authentication inside application.rbs"
      # response = RestClient.post("http://192.168.2.22:3000/api/v1/tokens.json", {:email => username, :password => password}.to_json, :content_type => :json, :accept => :json)
      response = RestClient.post "http://192.168.2.22:3000/tokens", {:email => username, :password => password}.to_json, :content_type => :json, :accept => :json
